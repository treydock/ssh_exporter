...
func NewCollector(target *config.Target, logger log.Logger) *Collector {
	return &Collector{
		...
		Timeout: prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "failed_due_to_timeout"),
			"Indicates the failure was due to timeout", nil, nil),
		Error: prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "failed_due_to_error"),
			"Indicates the failure was due to an error", nil, nil),
		CommandError: prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "failed_due_command_error"),
			"Indicates the failure was due to an error executed the configured command", nil, nil),
		CommandExpectFail: prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "failed_due_to_command_output"),

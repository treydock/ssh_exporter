...
func (c *Collector) Collect(ch chan<- prometheus.Metric) {
	level.Debug(c.logger).Log("msg", "Collecting SSH metrics", "target", c.target.Host)
...
func (c *Collector) collect() Metric {
	...
	if c.target.PrivateKey != "" {
		...
		if autherror != nil {
			...
			level.Error(c.logger).Log("msg", "Error setting up private key auth", "err", autherror, "target", c.target.Host)
			return metric
		}
	} else {
	...
	if err != nil {
		...
		level.Error(c.logger).Log("msg", "Failed to establish SSH connection", "err", err, "target", c.target.Host)
		return metric
	}
	...
	case <-time.After(time.Duration(c.target.Timeout) * time.Second):
		...
		close(c1)
		metric.FailureReason = "timeout"
		level.Error(c.logger).Log("msg", "Timeout establishing SSH session", "target", c.target.Host)
		return metric
	}
	close(c1)
	if sessionerror != nil {
		metric.FailureReason = "error"
		level.Error(c.logger).Log("msg", "Error establishing SSH session", "err", sessionerror, "target", c.target.Host)
		return metric
	}
	if commanderror != nil {
		metric.FailureReason = "command-error"
		level.Error(c.logger).Log("msg", "Error executing command", "err", commanderror, "target", c.target.Host, "command", c.target.Command)
		return metric
	}
	if c.target.Command != "" && c.target.CommandExpect != "" {
		...
		if !commandExpectPattern.MatchString(commandOutput) {
			level.Error(c.logger).Log("msg", "Command output did not match expected value",
				"output", commandOutput, "target", c.target.Host, "command", c.target.Command)
